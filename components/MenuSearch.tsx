...
export function MenuSearch({ sections }: MenuSearchProps) {
    ...
    return sections
      .map((section) => {
        ...
        const items = section.items.filter((item) => {
          const text = `${item.id} ${item.name} ${item.price} ${
            item.description ?? ""
          }`.toLowerCase();
  ...
  }, [query, sections, spicyOnly]);

  return (
    <div className="grid gap-5 lg:grid-cols-[15rem_1fr] lg:items-start">
      <aside className="sticky top-[7.5rem] z-20 rounded-2xl border border-[var(--warm-border)] bg-[var(--cream-paper)] p-3 shadow-lg shadow-green-950/5 lg:top-28">
        <MenuCategoryNav sections={sections} />
      </aside>

      <div>
        <div className="sticky top-[7.5rem] z-30 mb-5 rounded-2xl border border-[var(--warm-border)] bg-[var(--cream-paper)] p-3 shadow-lg shadow-green-950/5 lg:top-28">
